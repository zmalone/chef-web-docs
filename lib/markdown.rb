...
module ZurbFoundation
  ...
  def alerts
    ...
    content.gsub!(/<p>\[CALLOUT (.+)\] (.+)<\/p>/) {
      images = $1.split(',').map {|image|
        "<img class='alert-box-icon-large' src='/assets/images/#{image}'></img>"
      }
      "<div class='alert-box callout'>#{images.join}&nbsp; #{$2}</div>"
    }
  end
  ...
  def anchors
    content.gsub!(/<h([0-9])>(.*)<\/h[0-9]>/) do
      ...
      s + "<h#{size}>#{old}<span class=\"section-links\"><a class=\"section-link\" name=\"#{escaped}\" href=\"##{step || escaped}\"><i class=\"fa fa-paragraph\"></i></a><a class=\"section-link\" name=\"#top\" href=\"#top\"><i class=\"fa fa-long-arrow-up\"></span></i></a></h#{size}>"
    end
  end
